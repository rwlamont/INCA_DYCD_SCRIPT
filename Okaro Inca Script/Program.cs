...
namespace Okaro_Inca_Script
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Puts the data read in into inca string objects and then finally into a list
            ...
            //Sets up the Meta details at the top of the file
            ...
            writer.WriteLine(reader.ReadLine());
            int counter = 6;
            // Does the first 755 lines of the file that have only the two stream inflows
            foreach (IncaStrings inca in data)
            {
                if (counter > 755) break;
                counter++;
            ...
            //Adds in the data once the inflow file changes from two inflows to three
            for (int z = counter; z <= data.Count(); )
            {

                string line = reader.ReadLine();
                if (line == null) break;
                string[] splitLine = line.Split('\t');
                writer.WriteLine(splitLine[0] + seperator + splitLine[1] + seperator + data[z].Vol + seperator + splitLine[3] + seperator + splitLine[4] + seperator + splitLine[5] + seperator + data[z].NO3 + seperator + data[z].NH4 + seperator + splitLine[8] + seperator + splitLine[9] + seperator + splitLine[10] + seperator + splitLine[11] + seperator + splitLine[12] + seperator + splitLine[13]); // needs to go to 13
                z++;
